...
class Basic(Cog):
    ...
    async def start(self, ctx):
        ...
        await ctx.send(
            f"Welcome to your Kingdom! Try `{prefix}kingdom` to see your kingdom."
        )
    ...
    async def kingdom(self, ctx, user=None):
        ...
        balance, kingdom_name, kingdom_emblem = db.record(
            "SELECT Balance, KingdomName, KingdomEmblem\
    ...
    async def emblem(self, ctx, kingdom_emblem: Union[Emoji, str]):
        ...
        if isinstance(kingdom_emblem, str):
            await ctx.send(
                "This command only takes <custom emoji> as an argument! The custom emoji must be from this server."
            )
            return
        userID = ctx.author.id
    ...
    async def pay(self, ctx, user, amount: int = None):
        ...
        await ctx.send(
            f"{ctx.author.mention} has paid {receiver.mention} {amount} coins."
        )
