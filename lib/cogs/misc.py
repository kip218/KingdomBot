...
class Misc(Cog):
    ...
    async def snipe(self, ctx):
        ...
        embed = Embed(description=msg["content"], timestamp=msg["time"]).set_author(
            name=msg["author"], icon_url=msg["avatar_url"]
        )
    ...
    async def define(self, ctx, *, phrase):
        ...
        best_def = defs[0]
        embed = Embed(
            title=best_def.word, description=f"*{best_def.definition}*"
    ...
    async def remind(self, ctx, time: str, *, task):
        ...
        TIME_REGEX = re.compile(
            r"(([0-2]?\d|30):([0-1]?\d|2[0-3]):([0-5]?\d):([0-5]?\d)$)|(([0-1]?\d|2[0-3]):([0-5]?\d):([0-5]?\d)$)|(([0-5]?\d):([0-5]?\d)$)|(([0-5]?\d)$)"
        )
        match = TIME_REGEX.match(time)

        if not match:
            await ctx.send(
                "Please enter a valid time. Time should be formatted as 'dd:hh:mm:ss'."
            )
            return
        ...
        reminderID = int(
            str(int(now.timestamp()))[-9:] + str(int(reminderTime.timestamp()))[-9:]
        )

        db.add_reminder(reminderID, task, reminderTime, ctx.author.id, ctx.channel.id)
        await ctx.send(
            f"{ctx.author.mention} I will remind you of `{task}` after {d}d {h}h {m}m {s}s."
        )
    ...
    async def check_reminders(self):
        ...
        for reminder in reminders:
            try:
                reminderID, task, userID, channelID = (
                    reminder[0],
                    reminder[1],
                    reminder[2],
                    reminder[3],
    ...
    async def avatar(self, ctx, *, user: str = None):
        ...
        if user is None:
            ...
            while found is False and ind < len(lst_members):
                ...
                if user.lower() in (
                    curr_member.name.lower() + "#" + curr_member.discriminator.lower()
                ):
                    member = curr_member
                    found = True
            ...
            if found is False:
                await ctx.send(
                    'Could not find user named "' + user + '" in the server.'
                )
            else:
                embed = get_pfp(member)
                await ctx.send(embed=embed)
