...
class Game(Cog):
    ...
    async def montyhall(self, ctx):
        ...
        '''
        def check_guess(msg):
            return msg.author == ctx.author and msg.content in ['1', '2', '3'] and msg.channel == ctx.channel

        def check_switch(msg):
            return msg.author == ctx.author and msg.content.lower() in ['y', 'n'] and msg.channel == ctx.channel
    ...
    async def typeracer(self, ctx, num_words: int=5):
        ...
        def check_answer(msg):
            return (msg.content == t.get_word() or msg.content == t.get_word_display()) and msg.channel == ctx.channel
        ...
        while not t.game_over():
            ...
            #take answer
            try:
                answer = await self.bot.wait_for('message', check=check_answer, timeout=25)
            except TimeoutError:
                await msg.edit(content=msg.content + '\n\nThe game has timed out!')
                ...
                if t.is_correct(answer.content):
                    ...
                    await ctx.send(f"{answer.author.mention} Please don't copy and paste the word!")
