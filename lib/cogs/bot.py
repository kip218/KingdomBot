...
class Bot(Cog):
    ...
    async def servers(self, ctx):
        ...
        await ctx.send(
            f"Currently present in {num} different servers with {total_users} users."
        )
    ...
    async def server(self, ctx):
        ...
        embed = Embed(title=title, description=description, color=color).set_thumbnail(
            url=icon_url
        )
    ...
    async def invite(self, ctx):
        ...
        embed = Embed(title=title, url=url).set_author(
            name=description, icon_url=avatar_url
        )
        await ctx.send(embed=embed)
